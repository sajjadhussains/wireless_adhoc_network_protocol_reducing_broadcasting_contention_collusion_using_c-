...
vector<int> ODP::RunODP(int sourceNodeID, Network network)
{
    map<int, vector<int>> forwardListOfNodes;
    vector<int> forwardList;
    vector<int> forwardListOfODP;
...
// Source node broadcast the message
    forwardList = RunODPForSourceNode(sourceNodeID, network);
        ...
        if(itr != m_nodeTransmitterStatus.end() == true)
        {
            if(itr->second == messageReceivedFlag)
            {
                ...
                else
                {
                    ...
                    forwardList = RunODPForSourceDestinationNode(u[i], v[i], network);
...
// Extracting Forward List
    for( itr = m_nodeTransmitterStatus.begin(); itr != m_nodeTransmitterStatus.end(); itr++)
    {
        ...
        forwardListOfODP.push_back(itr->first);
    }

// Updating receiver status
    UpdateReceiverStatus(sourceNodeID, forwardListOfODP, network);
...
//    cout << "Number of collision is " << numberOfCollision << endl;
    ...
    return forwardListOfODP;
}

float ODP::GetNumberOfContention(Network network)
{
...
float ODP::GetNumberOfCollision(Network network)
{
    map<int, int>::iterator it;
    int maximumFlag = GetMaximumFlag(m_nodeTransmitterStatus);
...
void ODP::UpdateReceiverStatus(int sourceNodeID, vector<int> forwardListOfODP, Network network)
{
    ...
    for(int i = 0 ; i < nodeIDList.size() ; i++)
    {
        ...
        vector<int> receivedFrom = emptySet.SetIntersection(oneHopNeighborNodeIDList, forwardListOfODP);
...
int ODP::GetMaximumFlag(map<int, int> m_nodeTransmitterStatus)
{
    int maximumFlag = 0;
    map<int, int>::iterator it;
    for(it = m_nodeTransmitterStatus.begin(); it != m_nodeTransmitterStatus.end(); ++it)
...
vector<int> ODP::RunODPForSourceNode(int sourceNodeID, Network network)
{
    ...
    {
        cout << "Node " << ineligibleOneHopNeighborNodeIDList[i] << " is inserted to m_nodeTransmitterStatus with flag "<< broadcastCancelFlagForODP << endl;
        m_nodeTransmitterStatus.insert({ineligibleOneHopNeighborNodeIDList[i], broadcastCancelFlagForODP});
    }

    return forwardList;
}

void ODP::SetPriority(vector<int> forwardList, Network network)
{
    map<int, int>::iterator it;
...
int ODP::GetPriority(int NodeID)
{
    map<int, int>::iterator it;
    int priority;
...
vector<int> ODP::GetPrioritizedForwardList(vector<int> forwardList)
{
    map<int, int>::iterator it;
...
bool ODP::CheckForwardNodeStatus()
{
    map<int, int>::iterator it;
    bool check;
    for( it = m_nodeTransmitterStatus.begin(); it != m_nodeTransmitterStatus.end(); it++)
...
vector<int> ODP::CreateForwardList(map<int, vector<int>> eligibleOneHopNeighborNodeIDMapK, vector<int> uncoveredNeighborNodeIDList, vector<int> coveredNeighborNodeIDList, vector<int> forwardList, Network network)
{
...
vector<int> ODP::RunODPForSourceDestinationNode(int sourceNodeID, int destinationNodeID, Network network)
{
...
int ODP::GetEligibleOneHopNeighborNodeIDCoveredMaximumNodeIDs(map<int, vector<int>> eligibleOneHopNeighborNodeIDMapK, Network network)
{
...
int ODP::GetNodeIDFromMessageReceived(int nodeID, Network network)
{
    map<int, int>::iterator it;
...
bool ODP::IsNeighbor(int nodeID1, int nodeID2, Network network)
{
    bool check;
...
bool ODP::HaveCommonNeighbors(int nodeID1, int nodeID2, Network network)
{
    bool check;
    int receivedNodeID1 = GetNodeIDFromMessageReceived(nodeID1, network);
    int receivedNodeID2 = GetNodeIDFromMessageReceived(nodeID2, network);
...
bool ODP::AreNodesNeighbor(int nodeID1, int nodeID2, Network network)
{
    bool check;
...
bool ODP::HasCommonNeighbors(int nodeID1, int nodeID2, Network network)
{
    bool check;
    int receivedNodeID1 = GetNodeIDFromMessageReceived(nodeID1, network);
    int receivedNodeID2 = GetNodeIDFromMessageReceived(nodeID2, network);
...
vector<int> ODP::GetCommonNeighbors(int nodeID1, int nodeID2, Network network)
{
...
void ODP::DataSheetOfODP()
{
    string outputFilename = "odp.csv";
...
void ODP::PerformanceMeasure(Network network)
{
    map<int, int>::iterator it, it2;
    vector<int> forwardListOfODP;
...
// Find number of forwarding nodes
    ...
    {
        if(it->second >= broadcastFlag)
        forwardListOfODP.push_back(it->first);
    }
    float Nf = forwardListOfODP.size();
...
// Find average number of packets received
    ...
    for(int i = 0 ; i < network.m_nodes.size() ; i++)
    {
        ...
        receivedFromNodeIDList = emptySet.SetIntersection(oneHopNeighborNodeIDList, forwardListOfODP);
...
// Energy consumption
    ...
    for(int i = 0; i < forwardListOfODP.size(); i++)
    {
        it2 = m_nodePriorities.find(forwardListOfODP[i]);
        if(it2 != m_nodePriorities.end() == true)
        priority = it2->second;
...
// Broadcast Latency
    ...
    cout << "Number of rebroadcast cycle of ODP " << m_rebroadcastcycle << endl;

    Tdelay = (m_broadcast * t_broadcast + m_rebroadcastcycle * t_rebroadcast) / 1000 ; // milisecond

//Reachability

    vector<int> receivedNodeIDListOfODP;
    for( it = m_nodeReceiverStatus.begin(); it != m_nodeReceiverStatus.end(); it++)
    {
        if(it->second == messageReceivedSuccessfully)
        receivedNodeIDListOfODP.push_back(it->first);
    }
    float R = receivedNodeIDListOfODP.size() / N;
