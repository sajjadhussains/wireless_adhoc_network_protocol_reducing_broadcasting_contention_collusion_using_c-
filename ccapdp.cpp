...
CCAPDP::CCAPDP()
{
    // Contention and Collision Aware Partial Dominant Pruning
}
void CCAPDP::DoDispose()
{
...
vector<int> CCAPDP::RunCCAPDP(int sourceNodeID, Network network)
{
    map<int, vector<int>> forwardListOfNodes;
    vector<int> forwardList;
    vector<int> forwardListOfCCAPDP;
...
// Source node broadcast the message
    forwardList = RunCCAPDPForSourceNode(sourceNodeID, network);
        ...
        if(itr != m_nodeTransmitterStatus.end() == true)
        {
            if(itr->second == messageReceivedFlag)
            {
                ...
                else
                {
                    ...
                    forwardList = RunCCAPDPForSourceDestinationNode(u[i], v[i], network);
...
// Extracting Forward List
    for( itr = m_nodeTransmitterStatus.begin(); itr != m_nodeTransmitterStatus.end(); itr++)
    {
        ...
        forwardListOfCCAPDP.push_back(itr->first);
    }

// Updating receiver status
    UpdateReceiverStatus(sourceNodeID, forwardListOfCCAPDP, network);
...
//    cout << "Number of collision is " << numberOfCollision << endl;
    ...
    return forwardListOfCCAPDP;
}

float CCAPDP::GetNumberOfContention(Network network)
{
...
float CCAPDP::GetNumberOfCollision(Network network)
{
    map<int, int>::iterator it;
    int maximumFlag = GetMaximumFlag(m_nodeTransmitterStatus);
...
void CCAPDP::UpdateReceiverStatus(int sourceNodeID, vector<int> forwardListOfCCAPDP, Network network)
{
    ...
    for(int i = 0 ; i < nodeIDList.size() ; i++)
    {
        ...
        vector<int> receivedFrom = emptySet.SetIntersection(oneHopNeighborNodeIDList, forwardListOfCCAPDP);
...
int CCAPDP::GetMaximumFlag(map<int, int> m_nodeTransmitterStatus)
{
    int maximumFlag = 0;
    map<int, int>::iterator it;
    for(it = m_nodeTransmitterStatus.begin(); it != m_nodeTransmitterStatus.end(); ++it)
...
vector<int> CCAPDP::RunCCAPDPForSourceNode(int sourceNodeID, Network network)
{
    ...
    {
        cout << "Node " << ineligibleOneHopNeighborNodeIDList[i] << " is inserted to m_nodeTransmitterStatus with flag "<< broadcastCancelFlagForCCAPDP << endl;
        m_nodeTransmitterStatus.insert({ineligibleOneHopNeighborNodeIDList[i], broadcastCancelFlagForCCAPDP});
    }

    return forwardList;
}

void CCAPDP::SetPriority(vector<int> forwardList, Network network)
{
    map<int, int>::iterator it;
...
int CCAPDP::GetPriority(int NodeID)
{
    map<int, int>::iterator it;
    int priority;
...
vector<int> CCAPDP::GetPrioritizedForwardList(vector<int> forwardList)
{
    map<int, int>::iterator it;
...
bool CCAPDP::CheckForwardNodeStatus()
{
    map<int, int>::iterator it;
    bool check;
    for( it = m_nodeTransmitterStatus.begin(); it != m_nodeTransmitterStatus.end(); it++)
...
vector<int> CCAPDP::CreateForwardList(map<int, vector<int>> eligibleOneHopNeighborNodeIDMapK, vector<int> uncoveredNeighborNodeIDList, vector<int> coveredNeighborNodeIDList, vector<int> forwardList, Network network)
{
...
vector<int> CCAPDP::RunCCAPDPForSourceDestinationNode(int sourceNodeID, int destinationNodeID, Network network)
{
    vector<int> oneHopNeighborNodeIDListOfSource; // N(u)
    vector<int> oneHopNeighborNodeIDListOfDestination; // N(v)
    vector<int> withinTwoHopNeighborNodeIDList; // N(N(v))
    vector<int> sourceDestinationCommonNeighborNodeIDList; // X
    vector<int> oneHopNeighborNodeIDListOfCommonSDnodes; // P = N(X)
    vector<int> uncoveredNeighborNodeIDList; // U(v)
    vector<int> eligibleOneHopNeighborNodeIDList;  // B(v)
    vector<int> ineligibleOneHopNeighborNodeIDList;  // N(v)-F
    ...
    withinTwoHopNeighborNodeIDList = network.GetWithinTwoHopNeighborNodeIDListWithoutSourceNode(destinationNodeID);  //N(N(v))
    ...
    sourceDestinationCommonNeighborNodeIDList = emptySet.SetIntersection(oneHopNeighborNodeIDListOfSource, oneHopNeighborNodeIDListOfDestination); // X = N(v) Intersection N(u)
    for(int i = 0 ; i < sourceDestinationCommonNeighborNodeIDList.size() ; i++)
    {
        vector<int> oneHopNeighbors = network.GetOneHopNeighborNodeIDListWithoutSourceNode(sourceDestinationCommonNeighborNodeIDList[i]);
        oneHopNeighborNodeIDListOfCommonSDnodes = emptySet.SetUnion(oneHopNeighborNodeIDListOfCommonSDnodes, oneHopNeighbors);
    }
    uncoveredNeighborNodeIDList = emptySet.SetDifference(withinTwoHopNeighborNodeIDList, oneHopNeighborNodeIDListOfSource, oneHopNeighborNodeIDListOfDestination, oneHopNeighborNodeIDListOfCommonSDnodes); // U(v) = N(N(v)) - N(u) - N(v) - P
    ...
    network.PrintSortedNodeIDs(withinTwoHopNeighborNodeIDList);
    cout << "Element of X : ";
    network.PrintSortedNodeIDs(sourceDestinationCommonNeighborNodeIDList);
    cout << "Element of P : ";
    network.PrintSortedNodeIDs(oneHopNeighborNodeIDListOfCommonSDnodes);
    cout << "Element of B : ";
    network.PrintSortedNodeIDs(eligibleOneHopNeighborNodeIDList);
    cout << "Element of U : ";
    network.PrintSortedNodeIDs(uncoveredNeighborNodeIDList);
    ...
    for( it = m_nodeTransmitterStatus.begin() ; it != m_nodeTransmitterStatus.end() ; it++)
    {
        ...
        {
            int priority = GetPriority(destinationNodeID);  // CCAPDP
            //int priority = 1;                             // OPDP
...
int CCAPDP::GetEligibleOneHopNeighborNodeIDCoveredMaximumNodeIDs(map<int, vector<int>> eligibleOneHopNeighborNodeIDMapK, Network network)
{
...
int CCAPDP::GetNodeIDFromMessageReceived(int nodeID, Network network)
{
    map<int, int>::iterator it;
...
bool CCAPDP::IsNeighbor(int nodeID1, int nodeID2, Network network)
{
    bool check;
...
bool CCAPDP::HaveCommonNeighbors(int nodeID1, int nodeID2, Network network)
{
    bool check;
    int receivedNodeID1 = GetNodeIDFromMessageReceived(nodeID1, network);
    int receivedNodeID2 = GetNodeIDFromMessageReceived(nodeID2, network);
...
bool CCAPDP::AreNodesNeighbor(int nodeID1, int nodeID2, Network network)
{
    bool check;
...
bool CCAPDP::HasCommonNeighbors(int nodeID1, int nodeID2, Network network)
{
    bool check;
    int receivedNodeID1 = GetNodeIDFromMessageReceived(nodeID1, network);
    int receivedNodeID2 = GetNodeIDFromMessageReceived(nodeID2, network);
...
vector<int> CCAPDP::GetCommonNeighbors(int nodeID1, int nodeID2, Network network)
{
...
void CCAPDP::DataSheetOfCCAPDP()
{
    string outputFilename = "ccapdp.csv";
...
void CCAPDP::PerformanceMeasure(Network network)
{
    map<int, int>::iterator it, it2;
    vector<int> forwardListOfCCAPDP;
...
// Find number of forwarding nodes
    ...
    {
        if(it->second >= broadcastFlag)
        forwardListOfCCAPDP.push_back(it->first);
    }
    float Nf = forwardListOfCCAPDP.size();
...
// Find average number of packets received
    ...
    for(int i = 0 ; i < network.m_nodes.size() ; i++)
    {
        ...
        receivedFromNodeIDList = emptySet.SetIntersection(oneHopNeighborNodeIDList, forwardListOfCCAPDP);
...
// Energy consumption
    ...
    for(int i = 0; i < forwardListOfCCAPDP.size(); i++)
    {
        it2 = m_nodePriorities.find(forwardListOfCCAPDP[i]);
        if(it2 != m_nodePriorities.end() == true)
        priority = it2->second;
...
// Broadcast Latency
    ...
    cout << "Number of rebroadcast cycle of CCAPDP " << m_rebroadcastcycle << endl;

    Tdelay = (m_broadcast * t_broadcast + m_rebroadcastcycle * t_rebroadcast) / 1000 ; // milisecond

//Reachability

    vector<int> receivedNodeIDListOfCCAPDP;
    for( it = m_nodeReceiverStatus.begin(); it != m_nodeReceiverStatus.end(); it++)
    {
        if(it->second == messageReceivedSuccessfully)
        receivedNodeIDListOfCCAPDP.push_back(it->first);
    }
    float R = receivedNodeIDListOfCCAPDP.size() / N;
