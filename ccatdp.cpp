...
CCATDP::CCATDP()
{
    // Contention and Collision Aware Total Dominant Pruning
}

void CCATDP::DoDispose()
{
...
vector<int> CCATDP::RunCCATDP(int sourceNodeID, Network network)
{
    map<int, vector<int>> forwardListOfNodes;
    vector<int> forwardList;
    vector<int> forwardListOfCCATDP;
...
// Source node broadcast the message
    forwardList = RunCCATDPForSourceNode(sourceNodeID, network);
        ...
        if(itr != m_nodeTransmitterStatus.end() == true)
        {
            if(itr->second == messageReceivedFlag)
            {
                ...
                else
                {
                    ...
                    forwardList = RunCCATDPForSourceDestinationNode(u[i], v[i], network);
...
// Extracting Forward List
    for( itr = m_nodeTransmitterStatus.begin(); itr != m_nodeTransmitterStatus.end(); itr++)
    {
        ...
        forwardListOfCCATDP.push_back(itr->first);
    }

// Updating receiver status
    UpdateReceiverStatus(sourceNodeID, forwardListOfCCATDP, network);
...
//    cout << "Number of collision is " << numberOfCollision << endl;
    ...
    return forwardListOfCCATDP;
}

float CCATDP::GetNumberOfContention(Network network)
{
...
float CCATDP::GetNumberOfCollision(Network network)
{
    map<int, int>::iterator it;
    int maximumFlag = GetMaximumFlag(m_nodeTransmitterStatus);
...
void CCATDP::UpdateReceiverStatus(int sourceNodeID, vector<int> forwardListOfCCATDP, Network network)
{
    ...
    for(int i = 0 ; i < nodeIDList.size() ; i++)
    {
        ...
        vector<int> receivedFrom = emptySet.SetIntersection(oneHopNeighborNodeIDList, forwardListOfCCATDP);
...
int CCATDP::GetMaximumFlag(map<int, int> m_nodeTransmitterStatus)
{
    int maximumFlag = 0;
    map<int, int>::iterator it;
    for(it = m_nodeTransmitterStatus.begin(); it != m_nodeTransmitterStatus.end(); ++it)
...
vector<int> CCATDP::RunCCATDPForSourceNode(int sourceNodeID, Network network)
{
    ...
    {
        cout << "Node " << ineligibleOneHopNeighborNodeIDList[i] << " is inserted to m_nodeTransmitterStatus with flag "<< broadcastCancelFlagForCCATDP << endl;
        m_nodeTransmitterStatus.insert({ineligibleOneHopNeighborNodeIDList[i], broadcastCancelFlagForCCATDP});
    }

    return forwardList;
}

void CCATDP::SetPriority(vector<int> forwardList, Network network)
{
    map<int, int>::iterator it;
...
int CCATDP::GetPriority(int NodeID)
{
    map<int, int>::iterator it;
    int priority;
...
vector<int> CCATDP::GetPrioritizedForwardList(vector<int> forwardList)
{
    map<int, int>::iterator it;
...
bool CCATDP::CheckForwardNodeStatus()
{
    map<int, int>::iterator it;
    bool check;
    for( it = m_nodeTransmitterStatus.begin(); it != m_nodeTransmitterStatus.end(); it++)
...
vector<int> CCATDP::CreateForwardList(map<int, vector<int>> eligibleOneHopNeighborNodeIDMapK, vector<int> uncoveredNeighborNodeIDList, vector<int> coveredNeighborNodeIDList, vector<int> forwardList, Network network)
{
...
vector<int> CCATDP::RunCCATDPForSourceDestinationNode(int sourceNodeID, int destinationNodeID, Network network)
{
...
int CCATDP::GetEligibleOneHopNeighborNodeIDCoveredMaximumNodeIDs(map<int, vector<int>> eligibleOneHopNeighborNodeIDMapK, Network network)
{
...
int CCATDP::GetNodeIDFromMessageReceived(int nodeID, Network network)
{
    map<int, int>::iterator it;
...
bool CCATDP::IsNeighbor(int nodeID1, int nodeID2, Network network)
{
    bool check;
...
bool CCATDP::HaveCommonNeighbors(int nodeID1, int nodeID2, Network network)
{
    bool check;
    int receivedNodeID1 = GetNodeIDFromMessageReceived(nodeID1, network);
    int receivedNodeID2 = GetNodeIDFromMessageReceived(nodeID2, network);
...
bool CCATDP::AreNodesNeighbor(int nodeID1, int nodeID2, Network network)
{
    bool check;
...
bool CCATDP::HasCommonNeighbors(int nodeID1, int nodeID2, Network network)
{
    bool check;
    int receivedNodeID1 = GetNodeIDFromMessageReceived(nodeID1, network);
    int receivedNodeID2 = GetNodeIDFromMessageReceived(nodeID2, network);
...
vector<int> CCATDP::GetCommonNeighbors(int nodeID1, int nodeID2, Network network)
{
...
void CCATDP::DataSheetOfCCATDP()
{
    string outputFilename = "ccatdp.csv";
...
void CCATDP::PerformanceMeasure(Network network)
{
    map<int, int>::iterator it, it2;
    vector<int> forwardListOfCCATDP;
...
// Find number of forwarding nodes
    ...
    {
        if(it->second >= broadcastFlag)
        forwardListOfCCATDP.push_back(it->first);
    }
    float Nf = forwardListOfCCATDP.size();
...
// Find average number of packets received
    ...
    for(int i = 0 ; i < network.m_nodes.size() ; i++)
    {
        ...
        receivedFromNodeIDList = emptySet.SetIntersection(oneHopNeighborNodeIDList, forwardListOfCCATDP);
...
// Energy consumption
    ...
    for(int i = 0; i < forwardListOfCCATDP.size(); i++)
    {
        it2 = m_nodePriorities.find(forwardListOfCCATDP[i]);
        if(it2 != m_nodePriorities.end() == true)
        priority = it2->second;
...
// Broadcast Latency
    ...
    cout << "Number of rebroadcast cycle of CCATDP " << m_rebroadcastcycle << endl;

    Tdelay = (m_broadcast * t_broadcast + m_rebroadcastcycle * t_rebroadcast) / 1000 ; // milisecond

//Reachability

    vector<int> receivedNodeIDListOfCCATDP;
    for( it = m_nodeReceiverStatus.begin(); it != m_nodeReceiverStatus.end(); it++)
    {
        if(it->second == messageReceivedSuccessfully)
        receivedNodeIDListOfCCATDP.push_back(it->first);
    }
    float R = receivedNodeIDListOfCCATDP.size() / N;
