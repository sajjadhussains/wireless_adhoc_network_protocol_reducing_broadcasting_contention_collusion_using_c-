...
OPDP::OPDP()
{
    // Original Partial Dominant Pruning
}
...
vector<int> OPDP::RunOPDP(int sourceNodeID, Network network)
{
    map<int, vector<int>> forwardListOfNodes;
    vector<int> forwardList;
    vector<int> forwardListOfOPDP;
...
// Source node broadcast the message
    forwardList = RunOPDPForSourceNode(sourceNodeID, network);
        ...
        if(itr != m_nodeTransmitterStatus.end() == true)
        {
            if(itr->second == messageReceivedFlag)
            {
                ...
                else
                {
                    ...
                    forwardList = RunOPDPForSourceDestinationNode(u[i], v[i], network);
...
// Extracting Forward List
    for( itr = m_nodeTransmitterStatus.begin(); itr != m_nodeTransmitterStatus.end(); itr++)
    {
        ...
        forwardListOfOPDP.push_back(itr->first);
    }

// Updating receiver status
    UpdateReceiverStatus(sourceNodeID, forwardListOfOPDP, network);
...
//    cout << "Number of collision is " << numberOfCollision << endl;
    ...
    return forwardListOfOPDP;
}

float OPDP::GetNumberOfContention(Network network)
{
...
float OPDP::GetNumberOfCollision(Network network)
{
    map<int, int>::iterator it;
    int maximumFlag = GetMaximumFlag(m_nodeTransmitterStatus);
...
void OPDP::UpdateReceiverStatus(int sourceNodeID, vector<int> forwardListOfOPDP, Network network)
{
    ...
    for(int i = 0 ; i < nodeIDList.size() ; i++)
    {
        ...
        vector<int> receivedFrom = emptySet.SetIntersection(oneHopNeighborNodeIDList, forwardListOfOPDP);
...
int OPDP::GetMaximumFlag(map<int, int> m_nodeTransmitterStatus)
{
    int maximumFlag = 0;
    map<int, int>::iterator it;
    for(it = m_nodeTransmitterStatus.begin(); it != m_nodeTransmitterStatus.end(); ++it)
...
vector<int> OPDP::RunOPDPForSourceNode(int sourceNodeID, Network network)
{
    ...
    {
        cout << "Node " << ineligibleOneHopNeighborNodeIDList[i] << " is inserted to m_nodeTransmitterStatus with flag "<< broadcastCancelFlagForOPDP << endl;
        m_nodeTransmitterStatus.insert({ineligibleOneHopNeighborNodeIDList[i], broadcastCancelFlagForOPDP});
    }

    return forwardList;
}

void OPDP::SetPriority(vector<int> forwardList, Network network)
{
    map<int, int>::iterator it;
...
int OPDP::GetPriority(int NodeID)
{
    map<int, int>::iterator it;
    int priority;
...
vector<int> OPDP::GetPrioritizedForwardList(vector<int> forwardList)
{
    map<int, int>::iterator it;
...
bool OPDP::CheckForwardNodeStatus()
{
    map<int, int>::iterator it;
    bool check;
    for( it = m_nodeTransmitterStatus.begin(); it != m_nodeTransmitterStatus.end(); it++)
...
vector<int> OPDP::CreateForwardList(map<int, vector<int>> eligibleOneHopNeighborNodeIDMapK, vector<int> uncoveredNeighborNodeIDList, vector<int> coveredNeighborNodeIDList, vector<int> forwardList, Network network)
{
...
vector<int> OPDP::RunOPDPForSourceDestinationNode(int sourceNodeID, int destinationNodeID, Network network)
{
...
int OPDP::GetEligibleOneHopNeighborNodeIDCoveredMaximumNodeIDs(map<int, vector<int>> eligibleOneHopNeighborNodeIDMapK, Network network)
{
...
int OPDP::GetNodeIDFromMessageReceived(int nodeID, Network network)
{
    map<int, int>::iterator it;
...
bool OPDP::IsNeighbor(int nodeID1, int nodeID2, Network network)
{
    bool check;
...
bool OPDP::HaveCommonNeighbors(int nodeID1, int nodeID2, Network network)
{
    bool check;
    int receivedNodeID1 = GetNodeIDFromMessageReceived(nodeID1, network);
    int receivedNodeID2 = GetNodeIDFromMessageReceived(nodeID2, network);
...
bool OPDP::AreNodesNeighbor(int nodeID1, int nodeID2, Network network)
{
    bool check;
...
bool OPDP::HasCommonNeighbors(int nodeID1, int nodeID2, Network network)
{
    bool check;
    int receivedNodeID1 = GetNodeIDFromMessageReceived(nodeID1, network);
    int receivedNodeID2 = GetNodeIDFromMessageReceived(nodeID2, network);
...
vector<int> OPDP::GetCommonNeighbors(int nodeID1, int nodeID2, Network network)
{
...
void OPDP::DataSheetOfOPDP()
{
    string outputFilename = "opdp.csv";
...
void OPDP::PerformanceMeasure(Network network)
{
    map<int, int>::iterator it, it2;
    vector<int> forwardListOfOPDP;
...
// Find number of forwarding nodes
    ...
    {
        if(it->second >= broadcastFlag)
        forwardListOfOPDP.push_back(it->first);
    }
    float Nf = forwardListOfOPDP.size();
...
// Find average number of packets received
    ...
    for(int i = 0 ; i < network.m_nodes.size() ; i++)
    {
        ...
        receivedFromNodeIDList = emptySet.SetIntersection(oneHopNeighborNodeIDList, forwardListOfOPDP);
...
// Energy consumption
    ...
    for(int i = 0; i < forwardListOfOPDP.size(); i++)
    {
        it2 = m_nodePriorities.find(forwardListOfOPDP[i]);
        if(it2 != m_nodePriorities.end() == true)
        priority = it2->second;
...
// Broadcast Latency
    ...
    cout << "Number of rebroadcast cycle of OPDP " << m_rebroadcastcycle << endl;

    Tdelay = (m_broadcast * t_broadcast + m_rebroadcastcycle * t_rebroadcast) / 1000 ; // milisecond

//Reachability

    vector<int> receivedNodeIDListOfOPDP;
    for( it = m_nodeReceiverStatus.begin(); it != m_nodeReceiverStatus.end(); it++)
    {
        if(it->second == messageReceivedSuccessfully)
        receivedNodeIDListOfOPDP.push_back(it->first);
    }
    float R = receivedNodeIDListOfOPDP.size() / N;
