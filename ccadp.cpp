...
CCADP::CCADP()
{
    // Contention and Collision Aware Dominant Pruning
}

void CCADP::DoDispose()
{
...
vector<int> CCADP::RunCCADP(int sourceNodeID, Network network)
{
    map<int, vector<int>> forwardListOfNodes;
    vector<int> forwardList;
    vector<int> forwardListOfCCADP;
...
// Source node broadcast the message
    forwardList = RunCCADPForSourceNode(sourceNodeID, network);
        ...
        if(itr != m_nodeTransmitterStatus.end() == true)
        {
            if(itr->second == messageReceivedFlag)
            {
                ...
                else
                {
                    ...
                    forwardList = RunCCADPForSourceDestinationNode(u[i], v[i], network);
...
// Extracting Forward List
    for( itr = m_nodeTransmitterStatus.begin(); itr != m_nodeTransmitterStatus.end(); itr++)
    {
        ...
        forwardListOfCCADP.push_back(itr->first);
    }

// Updating receiver status
    UpdateReceiverStatus(sourceNodeID, forwardListOfCCADP, network);
...
//    cout << "Number of collision is " << numberOfCollision << endl;
    ...
    return forwardListOfCCADP;
}

float CCADP::GetNumberOfContention(Network network)
{
...
float CCADP::GetNumberOfCollision(Network network)
{
    map<int, int>::iterator it;
    int maximumFlag = GetMaximumFlag(m_nodeTransmitterStatus);
...
void CCADP::UpdateReceiverStatus(int sourceNodeID, vector<int> forwardListOfCCADP, Network network)
{
    ...
    for(int i = 0 ; i < nodeIDList.size() ; i++)
    {
        ...
        vector<int> receivedFrom = emptySet.SetIntersection(oneHopNeighborNodeIDList, forwardListOfCCADP);
...
int CCADP::GetMaximumFlag(map<int, int> m_nodeTransmitterStatus)
{
    int maximumFlag = 0;
    map<int, int>::iterator it;
    for(it = m_nodeTransmitterStatus.begin(); it != m_nodeTransmitterStatus.end(); ++it)
...
vector<int> CCADP::RunCCADPForSourceNode(int sourceNodeID, Network network)
{
    ...
    {
        cout << "Node " << ineligibleOneHopNeighborNodeIDList[i] << " is inserted to m_nodeTransmitterStatus with flag "<< broadcastCancelFlagForCCADP << endl;
        m_nodeTransmitterStatus.insert({ineligibleOneHopNeighborNodeIDList[i], broadcastCancelFlagForCCADP});
    }

    return forwardList;
}

void CCADP::SetPriority(vector<int> forwardList, Network network)
{
    map<int, int>::iterator it;
...
int CCADP::GetPriority(int NodeID)
{
    map<int, int>::iterator it;
    int priority;
...
vector<int> CCADP::GetPrioritizedForwardList(vector<int> forwardList)
{
    map<int, int>::iterator it;
...
bool CCADP::CheckForwardNodeStatus()
{
    map<int, int>::iterator it;
    bool check;
    for( it = m_nodeTransmitterStatus.begin(); it != m_nodeTransmitterStatus.end(); it++)
...
vector<int> CCADP::CreateForwardList(map<int, vector<int>> eligibleOneHopNeighborNodeIDMapK, vector<int> uncoveredNeighborNodeIDList, vector<int> coveredNeighborNodeIDList, vector<int> forwardList, Network network)
{
...
vector<int> CCADP::RunCCADPForSourceDestinationNode(int sourceNodeID, int destinationNodeID, Network network)
{
    ...
    uncoveredNeighborNodeIDList = emptySet.SetDifference(withinTwoHopNeighborNodeIDList, oneHopNeighborNodeIDListOfSource, oneHopNeighborNodeIDListOfDestination);  // U(v) = N(N(v)) - N(u) - N(v)
    ...
    for( it = m_nodeTransmitterStatus.begin() ; it != m_nodeTransmitterStatus.end() ; it++)
    {
        ...
        {
            int priority = GetPriority(destinationNodeID);  // CCADP
            //int priority = 1;                             //ODP
...
int CCADP::GetEligibleOneHopNeighborNodeIDCoveredMaximumNodeIDs(map<int, vector<int>> eligibleOneHopNeighborNodeIDMapK, Network network)
{
...
int CCADP::GetNodeIDFromMessageReceived(int nodeID, Network network)
{
    map<int, int>::iterator it;
...
bool CCADP::IsNeighbor(int nodeID1, int nodeID2, Network network)
{
    bool check;
...
bool CCADP::HaveCommonNeighbors(int nodeID1, int nodeID2, Network network)
{
    bool check;
    int receivedNodeID1 = GetNodeIDFromMessageReceived(nodeID1, network);
    int receivedNodeID2 = GetNodeIDFromMessageReceived(nodeID2, network);
...
bool CCADP::AreNodesNeighbor(int nodeID1, int nodeID2, Network network)
{
    bool check;
...
bool CCADP::HasCommonNeighbors(int nodeID1, int nodeID2, Network network)
{
    bool check;
    int receivedNodeID1 = GetNodeIDFromMessageReceived(nodeID1, network);
    int receivedNodeID2 = GetNodeIDFromMessageReceived(nodeID2, network);
...
vector<int> CCADP::GetCommonNeighbors(int nodeID1, int nodeID2, Network network)
{
...
void CCADP::DataSheetOfCCADP()
{
    string outputFilename = "ccadp.csv";
...
void CCADP::PerformanceMeasure(Network network)
{
    map<int, int>::iterator it, it2;
    vector<int> forwardListOfCCADP;
...
// Find number of forwarding nodes
    ...
    {
        if(it->second >= broadcastFlag)
        forwardListOfCCADP.push_back(it->first);
    }
    float Nf = forwardListOfCCADP.size();
...
// Find average number of packets received
    ...
    for(int i = 0 ; i < network.m_nodes.size() ; i++)
    {
        ...
        receivedFromNodeIDList = emptySet.SetIntersection(oneHopNeighborNodeIDList, forwardListOfCCADP);
...
*/
    ...
    for(int i = 0; i < forwardListOfCCADP.size(); i++)
    {
        it2 = m_nodePriorities.find(forwardListOfCCADP[i]);
        if(it2 != m_nodePriorities.end() == true)
        priority = it2->second;
...
// Broadcast Latency
    ...
    cout << "Number of rebroadcast cycle of CCADP " << m_rebroadcastcycle << endl;

    Tdelay = (m_broadcast * t_broadcast + m_rebroadcastcycle * t_rebroadcast) / 1000 ; // milisecond

//Reachability

    vector<int> receivedNodeIDListOfCCADP;
    for( it = m_nodeReceiverStatus.begin(); it != m_nodeReceiverStatus.end(); it++)
    {
        if(it->second == messageReceivedSuccessfully)
        receivedNodeIDListOfCCADP.push_back(it->first);
    }
    float R = receivedNodeIDListOfCCADP.size() / N;
